...
/**
 * 💡 初期データとカスタムデータをマージし、重複（同じチャンネルID）を自動排除して channels リストを更新する
 */
function updateChannelsList() {
  const rawChannels = [...window.initialChannels, ...customChannels];
  const uniqueMap = new Map();
  rawChannels.forEach(ch => uniqueMap.set(ch.id, ch));
